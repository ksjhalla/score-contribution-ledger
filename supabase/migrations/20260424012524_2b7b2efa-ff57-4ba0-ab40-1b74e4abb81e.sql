-- 1) Add status + settlement fields to work_entries
ALTER TABLE public.work_entries
  ADD COLUMN IF NOT EXISTS status text NOT NULL DEFAULT 'Pending',
  ADD COLUMN IF NOT EXISTS settled_amount numeric(14,2),
...
CREATE INDEX IF NOT EXISTS idx_work_entries_user_status
  ON public.work_entries (user_id, status);

-- 2) Extend the public passport function to include work-entry totals.
